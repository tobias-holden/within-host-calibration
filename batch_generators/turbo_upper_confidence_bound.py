...
class TurboUpperConfidenceBound(BatchGenerator):
    dim: int
    batch_size: int
    length: float = 1.0
    ...
    def __init__(self, dim=1, batch_size=4, failure_tolerance = None, beta = 0.1, num_restarts = 10, raw_samples = 512, dtype=torch.double, device=torch.device("cpu"), length_min=2**-30):
        super().__init__()
        self.dim = dim
