...
def translate_parameters(key, guesses, ps_id):
    ...
    if iflag=='CONSTANT_DISTRIBUTION':
      ...
      output.loc[output['parameter'] == 'InnateImmuneDistribution2', 'emod_value'] = 1.0
    elif iflag=='GAUSSIAN_DISTRIBUTION':
      change = output.loc[output['parameter'] == 'InnateImmuneDistribution2', 'emod_value']
      # Convert standard deviation to range between 0.01 and 0.3
      change = 0.01+change*(0.3-0.01)
      output.loc[output['parameter'] == 'InnateImmuneDistribution2', 'emod_value'] = change
      # Fix mean at 1.0
      output.loc[output['parameter'] == 'InnateImmuneDistribution1', 'emod_value'] = 1.0
    elif iflag=='EXPONENTIAL_DISTRIBUTION':
      change = output.loc[output['parameter'] == 'InnateImmuneDistribution1', 'emod_value']
      # Convert decay rate to range between 0.575 and euler's number (roughly 25-75% bound)
      change = 0.575+change*(2.71828182845-0.575)
      output.loc[output['parameter'] == 'InnateImmuneDistribution1', 'emod_value'] = change
      # No second parameter, set to zero.
      output.loc[output['parameter'] == 'InnateImmuneDistribution2', 'emod_value'] = 0
    elif iflag=='LOG_NORMAL':
      change = output.loc[output['parameter'] == 'InnateImmuneDistribution2', 'emod_value']
      # Convert standard deviation to range between 0.01 and 1
      change = 0.01+change*(1-0.01)
      output.loc[output['parameter'] == 'InnateImmuneDistribution2', 'emod_value'] = change
      # Fix mean at 0.0
      output.loc[output['parameter'] == 'InnateImmuneDistribution1', 'emod_value'] = 0.0
...
if __name__ == '__main__':
    ...
    team_default_params2 =  [0.235457679394, # Antigen switch rate (7.65E-10) 
                             ...
                             0.492373751573,  # Pyrogenic threshold (15000)
                             0,               # Max Individual Infections (3)
                             0.4,             # Erythropoesis Anemia Effect Size (3.5)
                             0.1,             # RBC Destruction Multiplier (3.9)
                             0.1,             # Cytokine Gametocyte Killing (0.02)
                             0,               # InnateImmuneDistributionFlag (UNIFORM)
                             0,               # InnateImmuneDistribution1 (0)
                             1]               # InnateImmuneDistribution2 (1)
                       
    print(translate_parameters(param_key,team_default_params2,1))
    
    test_params=np.random.uniform(0,1,20)
    print(translate_parameters(param_key,test_params,1))
