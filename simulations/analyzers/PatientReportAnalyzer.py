...
class PatientAnalyzer(IAnalyzer):
    ...
    def __init__(self, expt_name, working_dir='.', start_report_day=0, sweep_variables=None):
        super(PatientAnalyzer, self).__init__(working_dir=working_dir,
                                              filenames=['output/MalariaPatientReport.json']
                                              )

        self.expt_name = expt_name
        self.sweep_variables = sweep_variables or ['Run_Number', 'x_Temp_LH_values', 'Site']
    ...
    with Platform('Calculon') as platform:
        ...
        if use_ssmt:
            ...
            for expt_name, exp_id in experiments.items():
                ...
                analyzers = [PatientAnalyzer(working_dir=".", expt_name=expt_name, start_report_day=start_report_day, sweep_variables=sweep_variables)]
