...
class InsetChartAnalyzer(BaseAnalyzer):
    ...
    def reduce(self, all_data: Dict[Union[IWorkflowItem, Simulation], Any]) -> Any:
        ...
        df_prev_final.to_csv(os.path.join(output_dir, "sporozoite_reduction_prevalence_final.csv"), index=False)
        df_inc_final.to_csv(os.path.join(output_dir, "sporozoite_reduction_incidence_final.csv"), index=False)
