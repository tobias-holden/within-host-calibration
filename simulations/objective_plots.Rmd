...
```{r}
bloodless <- scores %>% filter(metric=="no_blood") %>%
  select(param_set,round,site,metric,ll) %>% 
  group_by(param_set,round) %>%
  summarize(b=sum(ll)) %>%
  ungroup() %>%
  filter(b!=0)

total_scores %>% arrange(-total_score) %>% 
  select(param_set,round) %>% 
  mutate(label="top") %>% head(5) -> top

total_scores %>% filter(param_set == 1 & round==0) %>% 
  select(param_set,round) %>% 
  mutate(label="default") -> def

scores %>% 
  filter(!(interaction(param_set,round) %in% interaction(bloodless$param_set,bloodless$round))) %>%
  filter(metric!="no_blood") %>% 
  group_by(site,metric) %>% filter(ll==max(ll)) %>%
  ungroup() %>% 
  mutate(label=interaction(site,metric))%>%
  select(param_set,round,label) -> local

improvements %>% 
  select(param_set,round) %>% 
  mutate(label=interaction(param_set,round,lex.order = T)) -> progressive

#####

selection_type = "final" # [progress, final]

#####

if(selection_type=="progress") {
  particles <- progressive %>% filter(!(interaction(round,param_set) %in% interaction(bloodless$round,bloodless$param_set)))
} else if (selection_type == "final") {
  particles <- rbind.data.frame(top,
                                rbind.data.frame(local, def))
} else if (selection_type=="limited"){
  particles <- progressive %>% 
    filter(!(interaction(round,param_set) %in% interaction(bloodless$round,bloodless$param_set))) %>%
    filter(round==max(round)) %>%
    filter(param_set==max(param_set))%>%
    rbind.data.frame(def)
}

particles %>%
  group_by(param_set,round)%>%
  mutate(label2=paste(label,collapse=" + ")) %>%
  select(-label)%>%
  ungroup() %>%
  arrange(param_set,round) %>%
  mutate(dup=duplicated(interaction(param_set,round))) %>%
  filter(!dup) %>% select(-dup) %>% rename(label=label2) -> particles


if(selection_type=="progress"){
  particle_levels=interaction(particles$param_set,particles$round,drop = T)
  particle_levels=sort(particle_levels)
} else if(selection_type=="final"){
  particles$label = gsub(" \\+ ","\n",particles$label)
  particle_levels=unique(particles$label)
} else if(selection_type=="limited"){
  particle_levels=c("default",paste(interaction(best_ps,best_round)))
}

particles <- particles %>% filter(grepl("top",label) | grepl("default",label) | grepl("incidence",label))

#particles <-data.frame(param_set=c(1,2,3,4,5,6,best_ps),round=c(0,0,0,0,0,0,best_round),label=c("og3","og20","best20","og3ltd","og20ltd","best20ltd","best"))
#particles <- particles %>% rbind.data.frame(local)


# particles <-data.frame(param_set=c(1,2,3,4,5,6,7,8),
#                        round=c(0,0,0,0,0,0,0,0),
#                        label=c("og3","og20","og3LTD","og20LTD","best20","best20LTD","recent1","recent2"))
particle_levels=unique(particles$label)
  ...
  print("Hey")
    ...
    inc_ref %>% filter(Site == plot_site) -> inc_ref
      ...
      inc_df %>%
        ...
        group_by(Age,site,round,param_set)%>%
        summarize(Severe.Incidence=mean(Severe.Incidence))%>%
        #filter(round==max(round)) %>%
        ungroup()%>%
        filter(Age %in% inc_ref$Age) %>%
        group_by(param_set,round,site)%>%
        arrange(Age)%>%
        ...
        geom_line(aes(group=interaction(round,param_set),
                      ...
                      color=factor(label,levels=particle_levels),
...
plot_incidence <- function(plot_particles,score_df,plot_site){
  ...
  for(i in 1:nrow(ddmp))
  {
    ...
    tmp <- read.csv(paste("output/",exp,"/LF_",ddmp$round[i],
                          "/SO/",ddmp$site[i],"/prev_inc_by_age_annual.csv",sep=''))
    tmp %>% right_join(ddmp[i,]) -> tmp
    if(i==1){
      inc_df <- tmp 
    } else {
  ...
  inc_df %>% 
    filter(site==plot_site) %>% 
    group_by(round,param_set,site,Age,label) %>%
    summarize(Incidence=mean(Incidence)) %>%
    #mutate(label=ifelse(selection_type=="final",gsub(" \\+ ","\n",label),label)) %>%
    #mutate(label=ifelse(selection_type=="progress",label,label)) %>%
    ...
    scale_linewidth_manual(values=c(1,1)) +
    scale_size_continuous() +
    guides(alpha="none",shape="none",size=guide_legend(direction = "horizontal")) -> pp
  print(pp)
...
plot_annual_prevalence <- function(plot_particles,score_df,plot_site){
  ...
  annual_prev_df %>%
  group_by(Age,round,param_set,label) %>%
...
monthly_prev_df %>%
  ...
  group_by(site,month,agebin,param_set,round,label) %>% 
  ...
  for(i in 1:nrow(ddmp))
{
  ...
  ps=ddmp$param_set[i]
  lab=ddmp$label[i]
  print(paste(plot_site,glue("round ",round," parameter set ",ps),sep=" --- "))
  ...
  group_by(i,Site,round,param_set,month,agebin,densitybin,infectiousness_bin,label) %>%
  summarize(counts=sum(counts)) %>%
  group_by(i,Site,round,param_set,month,agebin,label) %>% mutate(total_count=sum(counts)) %>%
  ungroup() %>%
...
# Plotting
  s=plot_site
      ...
      inf_df %>%
        ...
        guides(size=guide_legend(ncol=2),alpha="none")+
        theme(legend.position="right",
...
ddmp <- particles %>% left_join(scores) %>% filter(site%in%inf_sites) %>% select(site,round,param_set,label)%>% distinct() 

ddmp <- ddmp %>% filter(grepl("default",label) | grepl("top",label) | grepl("infectiousness",label))
...
for(i in 1:nrow(ddmp))
{
  ...
  temp <- read.csv(paste("output/",exp,
          ...
          mutate(round=round) %>% mutate(label=ddmp$label[i])
...
inf_df %>%
  ...
  rename(site=Site) %>%
  right_join(inf_ref) %>%
  ggplot(aes(y=interaction(month,agebin,site),
             x=interaction(densitybin,infectiousness_bin))) +
  facet_wrap(~interaction(param_set,round)) +
  theme_minimal(base_size=10)+
  geom_tile(color="white",
            aes(fill=sign(freq_frac_infect-infectiousness_bin_freq)*log10(abs(freq_frac_infect-infectiousness_bin_freq)))) +
...
n_sites <- length(unique(inf_df$Site))
n_dens_bins <- length(unique(inf_df$densitybin))
n_inf_bins <- length(unique(inf_df$infectiousness_bin))
n_age_bins <- length(unique(inf_df$agebin))
n_months <- length(unique(inf_df$month))

length_x = length(unique(interaction(inf_df$densitybin,inf_df$Site,inf_df$month)))
length_y = length(unique(interaction(inf_df$agebin,inf_df$infectiousness_bin)))

inf_df%>%
    filter(interaction(month,agebin,Site) %in% interaction(inf_ref$month,inf_ref$agebin,inf_ref$site)) %>%
    filter(Pop == max(Pop)) %>%
    mutate(counts = infectiousness_bin_freq * Pop) %>%
    group_by(i,Site,round,param_set,month,agebin,densitybin,infectiousness_bin,label) %>%
    summarize(counts=sum(counts)) %>%
    group_by(i,Site,round,param_set,month,agebin,label) %>% mutate(total_count=sum(counts)) %>%
    ungroup() %>%
    mutate(freq_frac_infect=counts/total_count) %>%
    left_join(inf_ref %>% rename(Site=site, ref_frac=freq_frac_infect, 
                                 infectiousness_bin=fraction_infected_bin)) %>%
    ggplot(aes(x=interaction(factor(densitybin),month.abb[month],Site),y=interaction(factor(infectiousness_bin),agebin)))+
    geom_vline(xintercept=seq(0.5,length_x,n_dens_bins-1)) + # between months
    geom_hline(yintercept=seq(0.5,length_y,n_inf_bins))+
    facet_grid(label~.) +
#paste(Site,'\n',month.name[month])) +
    geom_tile(data=.%>%filter(freq_frac_infect>0.02 | ref_frac>0),
...
inf_df%>%
    ...
    left_join(inf_ref %>% rename(Site=site, ref_frac=freq_frac_infect, 
                                 infectiousness_bin=fraction_infected_bin)) %>%
    ggplot(aes(x=interaction(factor(densitybin),month.abb[month],Site),y=interaction(factor(infectiousness_bin),agebin)))+
    #geom_vline(xintercept=seq(0.5,length_x,n_dens_bins),linewidth=0.2,linetype=2) + # between months
    geom_hline(yintercept=seq(0.5,length_y,n_inf_bins),linewidth=0.2,linetype=2)+
    facet_grid(label~Site,scales="free_x") +
#paste(Site,'\n',month.name[month])) +
    theme_minimal(base_size=10) +
    theme(axis.text.x=element_text(angle=-90,vjust=0,hjust=0)) +
    #scale_color_distiller(palette="Spectral") +
    ...
    geom_point(data=.%>%filter(freq_frac_infect>0),
               aes(color=label, size=freq_frac_infect),
                   alpha=0.5) +
    geom_point(data=inf_ref %>% 
               #filter(densitybin>0)%>%
               filter(freq_frac_infect>0)%>%
               mutate(infectiousness_bin=fraction_infected_bin,
                      Site=site),
...
inf_df %>% 
  ...
  facet_wrap(site~interaction(label)) 
...
for(i in 1:nrow(ddmp)){
  ...
  inf_df%>%
    ...
    #scale_color_distiller(palette="Spectral") +
    scale_fill_distiller(palette="Spectral") +
    ggtitle(paste(gsub('.{5}$', '',s),"round",r,"#",ps),lab) -> p
  print(p)
  ...
  inf_df %>%
    ...
    ggtitle(paste(gsub('.{5}$', '',s),"round",r,"#",ps),lab) +
...
# Prevalence vs. Incidence Curves

```{r}
oos_prev_inc <- read_csv("~/b1139/within-host-calibration/reference_datasets/oos_prev_inc.csv")

oos_prev_inc %>%
  ggplot(aes(x=pfpr,y=incidence)) +
  geom_point() +
