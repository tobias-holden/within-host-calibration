...
```{r}
particles <- total_scores %>% 
              arrange(-total_score) %>% 
              filter(round!=0) %>% head(3) %>% 
...
plot_incidence <- function(plot_particles,score_df,plot_site){
  ...
  inc_df %>%
    ...
    ggtitle(toupper(plot_site))+
    theme_minimal(base_size=16) +
    geom_line(aes(group=interaction(round,param_set),color=interaction(round,param_set))) +
...
plot_annual_prevalence <- function(plot_particles,score_df,plot_site){
  ...
  labs(color=NULL, shape=NULL,size="sample size") +
  guides(linewidth="none")+
  ggtitle(plot_site) +
  theme_minimal(base_size=16) +
