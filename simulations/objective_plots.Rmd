...
best_sets %>% left_join(total_scores %>% ungroup() %>% mutate(rank=rank(-total_score))) %>%
  ggplot(aes(x=unit_value,y=dist)) +
  facet_wrap(~parameter,scales="free_x") +
  geom_point(aes(color=factor(rank)))

particles <- best_sets %>% select(round,param_set) %>% distinct()


best_sets %>% left_join(scores) %>%
  ggplot(aes(x=gsub("-\n","",
                    gsub("Log10","",
                         gsub(" ","\n",
                              gsub("  ","\n",
                                   factor(parameter,levels=unique(best_sets$parameter)[param_order]))))),y=unit_value)) +
  geom_line(aes(group=interaction(round,param_set)))+
  coord_polar() 

best_sets %>% ungroup() %>% left_join(parameters) %>% select(param_set,round,parameter,unit_value) %>%
  pivot_wider(names_from = parameter,values_from=unit_value) %>%
  select(-c(param_set,round)) %>%
  kmeans(centers=2) -> clustered

best_sets %>% left_join(parameters) %>% select(param_set,round,parameter,unit_value) %>%
  pivot_wider(names_from = parameter,values_from=unit_value) %>% ungroup() -> bs_cluster

bs_cluster$cluster=clustered$cluster


#GGally::ggpairs(bs_cluster %>% select(-c(param_set,round)) %>% relocate(cluster),mapping = aes(color=factor(cluster)))

clusters <- best_sets %>% left_join(bs_cluster %>% select(param_set,round,cluster)) %>% select(param_set,round,cluster) %>% distinct()


bs_cluster %>%
  gather("parameter","unit_value",-c(param_set,round,cluster)) %>%
  mutate(parameter=gsub("-\n","",
                    gsub("Log10","",
                         gsub(" ","\n",
                              gsub("  ","\n",
                                   factor(parameter,levels=unique(best_sets$parameter)[param_order])))))) -> bs_cluster
bs_cluster %>%  
  ggplot(aes(x=factor(parameter,levels=unique(bs_cluster$parameter)[param_order]),
             y=unit_value)) +
  geom_line(aes(group=interaction(round,param_set),
...
#   select(-total_score) -> particles

best_sets %>% left_join(scores) %>% select(round,parameter,site,metric,ll,my_weight) %>%
  group_by(round,param_set,site,metric)%>%summarize(weighted_ll=ll*my_weight) %>%
  ungroup() %>% distinct() %>%
mutate(objective=interaction(metric,site)) %>%
  select(-c(site,metric)) %>%
  pivot_wider(names_from = objective,values_from=weighted_ll) %>% ungroup() -> score_cluster

score_cluster %>% select(-c(param_set,round)) %>% 
  kmeans(centers=3) -> clustered

score_cluster$cluster <- clustered$cluster

clusters <- best_sets %>% left_join(score_cluster %>% select(param_set,round,cluster)) %>% select(param_set,round,cluster) %>% distinct()


score_cluster %>%
  gather("objective","score",-c(param_set,round,cluster)) %>% 
  mutate(objective=gsub("\\.","\n",objective))-> score_cluster

score_cluster %>%  
  filter(!grepl("no_blood",objective))%>%
  ggplot(aes(x=factor(objective),
             y=-log10(-score))) +
  geom_line(aes(group=interaction(round,param_set),
                color=factor(cluster)))+
  coord_polar() +
  ...
  print("Hey")
    ...
    inc_ref %>% filter(Site == plot_site) -> inc_ref
      ...
      inc_df %>%
        ...
        geom_line(aes(group=interaction(param_set,round),
                      #color=ifelse(round<training_batches,"gray",my_pal[round+1]),
                      color=interaction(cluster),
...
plot_incidence <- function(plot_particles,score_df,plot_site){
  ...
  inc_df %>%
    ...
    ggtitle(toupper(plot_site))+
    theme_minimal(base_size=16) +
    geom_line(aes(group=interaction(round,param_set),color=factor(cluster)),
                  linewidth=1) +
...
monthly_prev_df %>%
  ...
  geom_line(aes(group=interaction(round,param_set,lex.order = T),
                color=interaction(param_set,round)),linewidth=1) +
...
# Plotting
  s=plot_site
      ...
      inf_df %>%
        ...
        geom_point(aes(color=interaction(param_set,round)),
                   alpha=0.3) +
