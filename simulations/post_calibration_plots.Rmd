...
if(length(severe_incidence_sites)>0){
  ...
  ddmp <- scores %>% filter(site %in% severe_incidence_sites & metric=="severe_incidence")
...
ddmp <- scores %>% filter(site %in% incidence_sites & metric=="incidence")
...
ddmp <- scores %>% filter(site =="namawala_2001" & metric=="prevalence")
...
ddmp <- scores %>% filter(site %in% mp_sites & metric=="prevalence")
...
ddmp <- scores %>% filter(site%in%density_sites) %>% select(site,round,param_set)%>% distinct()
...
ddmp <- scores %>% filter(site%in%inf_sites) %>% select(site,round,param_set)%>% distinct()
