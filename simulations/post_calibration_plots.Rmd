...
scores %>%
  ...
  ggsave(paste(score_folder,paste("scores_best_sets.pdf",sep="_"),sep='/'),bp1,height =12,width=8)
...
tp %>%
  ...
  pp
  ggsave(paste(parameters_folder,"emod_values.pdf",sep='/'),pp,height = 12,width=10)
...
severe_incidence_sites <- sim_coord_df$site[sim_coord_df$age_severe_incidence==1 & sim_coord_df$include_site]

if(length(severe_incidence_sites)>0){
  ddmp <- improvements %>% left_join(scores) %>% filter(site %in% severe_incidence_sites)
  ddmp <- scores %>% filter(site %in% severe_incidence_sites & param_set!=4 & metric=="severe_incidence")
...
for(s in unique(inc_df$site)){
  ...
  ggtitle(toupper(s))+
  theme_minimal(base_size=16) +
  geom_line(aes(group=interaction(round,param_set,Run_Number),
  ...
  print(pp)
  ggsave(paste(objectives_folder,paste(s,"severe_incidence_astmh.pdf",sep="_"),sep='/'),pp,height = 6,width=6)
  ...
  guides(alpha="none",shape="none") -> pp
  print(pp)
  ggsave(paste(objectives_folder,paste(s,"severe_incidence_best.pdf",sep="_"),sep='/'),pp,height = 6,width=6)
  ...
  print(pp)
  ggsave(paste(objectives_folder,paste(s,"severe_incidence.pdf",sep="_"),sep='/'),pp,height = 6,width=6)
}
...
ddmp <- improvements %>% left_join(scores) %>% filter(site %in% incidence_sites & metric=="incidence")
ddmp <- scores %>% filter(site %in% incidence_sites & metric=="incidence" & param_set!=4)
ddmp <- top100 %>% left_join(scores) %>% filter(site %in% incidence_sites & metric=="incidence")
...
for(s in unique(inc_df$site)){
  ...
  guides(alpha="none",shape="none") -> pp
  print(pp)
  ggsave(paste(objectives_folder,paste(s,"incidence_best.pdf",sep="_"),sep='/'),pp,height = 6,width=6)
  ...
  scale_linetype_manual(values=c(2,rep(1,length(unique(inc_df$round)))))-> pp
  print(pp)
  ggsave(paste(objectives_folder,paste(s,"incidence.pdf",sep="_"),sep='/'),pp,height = 6,width=6)
}
...
inc_df %>%
  ...
  geom_path(aes(group=interaction(round,param_set),
                color=my_pal[round+1]),
            size=1.5)+
  geom_path(data=inc_ref %>% mutate(site=Site), aes(y=INC/1000)) +
  geom_point(data=inc_ref %>% mutate(site=Site), aes(y=INC/1000,size=POP)) +
  ylab("Cases per person per year")+
  scale_color_identity() +
  scale_size_continuous(breaks=seq(10,100,20))+
  labs(size="Sample size") +
  scale_x_log10()+
  scale_y_continuous(minor_breaks = seq(0,10,0.5),breaks=seq(0,10,1))+
...
ddmp <- improvements %>% left_join(scores) %>% filter(site =="namawala_2001" & metric=="prevalence")
ddmp <- scores %>% filter(site =="namawala_2001" & metric=="prevalence" & param_set!=4)
ddmp <- top100 %>% left_join(scores) %>% filter(site =="namawala_2001" & metric=="prevalence")
...
for(i in 1:nrow(ddmp))
{
  ...
  tmp <- read.csv(paste("output/",exp,"/LF_",ddmp$round[i],
                 "/SO/namawala_2001/inc_prev_data_full.csv",sep=''))
  tmp %>% right_join(ddmp[i,]) -> tmp
  if(i==1){
...
for(r in unique(namawala_prev_df$round)){
  ...
  ggsave(paste(objectives_folder,paste("Namawala_prevalence.pdf",sep="_"),sep='/'),pp,height = 8,width=8)
}
...
namawala_prev_df %>%
  ...
  scale_x_continuous(breaks = unique(namawala_prev_df$Age),minor_breaks = NULL) ->pp
  pp
  ggsave(paste(objectives_folder,paste("Namawala_prevalence_best.pdf",sep="_"),sep='/'),pp,height = 8,width=8)
...
# Gather simulation outputs
ddmp <- improvements %>% left_join(scores) %>% filter(site %in% mp_sites & metric=="prevalence")
ddmp <- scores %>% filter(site %in% mp_sites & metric=="prevalence" & param_set!=4)
ddmp <- top100%>%left_join(scores) %>% filter(site %in% mp_sites & metric=="prevalence")
...
garki_prev_ref<-garki_prev_ref %>% filter(tolower(Site) %in% tolower(garki_prev_df$site))
...
garki_prev_df %>%
  ...
  pp  
ggsave(paste(objectives_folder,paste("Garki_prevalence_best.pdf",sep="_"),sep='/'),pp,height = 8,width=8)
...
for(r in unique(garki_prev_df$round)){
  ...
  theme(legend.position="bottom") +
  labs(size="sample size") -> pp
  ggsave(paste(objectives_folder,paste("Garki_prevalence.pdf",sep="_"),sep='/'),pp,height = 8,width=8)
...
# Gather simulation outputs
ddmp <- improvements %>% left_join(scores) %>% filter(site%in%density_sites) %>% select(site,round,param_set)%>% distinct()

ddmp <- scores %>% filter(site%in%density_sites & param_set!=4) %>% select(site,round,param_set)%>% distinct()
...
for(s in unique(dens_df$Site)){
  ...
  if(s=="dapelogo_2007"){
     ...
     dens_df %>%
      ...
      geom_line(aes(group=interaction(round,param_set),
                     color=my_pal[as.integer(round)+1]),size=1.5)+
      ...
      print(pp)
      ggsave(paste(objectives_folder,paste(s,"asexual_densities.pdf",sep="_"),sep='/'),pp,height = 8,width=8)
     ...
     dens_df %>%
      ...
      print(pp)
      ggsave(paste(objectives_folder,paste(s,"gametocyte_densities.pdf",sep="_"),sep='/'),pp,height = 8,width=8)
     ...
     ref_dens <- ref_dens %>% filter(tolower(site)==tolower(s))
     for(ds in unique(ref_dens$site)){
      ...
      theme(legend.position = "none") -> pp
       print(pp)
      ggsave(paste(objectives_folder,paste(ds,"asexual_densities_astmh.pdf",sep="_"),sep='/'),pp,height = 8,width=8)
      ...
      theme(legend.position = "none") -> pp
      print(pp)
      ggsave(paste(objectives_folder,paste(ds,"gameotcyte_densities_astmh.pdf",sep="_"),sep='/'),pp,height = 8,width=8)
     }
  }
  }
...
for(s in inf_sites[which(inf_sites %in% inf_df$Site)]){
      ...
      pp 
      ggsave(paste(objectives_folder,paste(s,"infectiousness.pdf",sep="_"),sep='/'),pp,height = 10,width=10)
}
...
for(i in 1:nrow(ddmp)){
  ...
  print(p)
    ggsave(paste(objectives_folder,paste(s,"infectiousness_plot1",r,ps,".pdf",sep="_"),sep='/'),p,height = 10,width=10)
  ...
  inf_df %>%
    ...
    ggtitle(paste(gsub('.{5}$', '',s),"round",r,"#",ps)) -> p
  print(p)
    ggsave(paste(objectives_folder,paste(s,"infectiousness_plot2",r,ps,".pdf",sep="_"),sep='/'),p,height = 10,width=10)
  ...
  inf_df %>%
    ...
    scale_size_continuous(range = c(0,2)) +
    ggtitle(paste(gsub('.{5}$', '',s),"round",r,"#",ps)) -> p
    ggsave(paste(objectives_folder,paste(s,"infectiousness_plot3",r,ps,".pdf",sep="_"),sep='/'),p,height = 10,width=10)
  ...
  print(p)
    ggsave(paste(objectives_folder,paste(s,"infectiousness_plot4",r,ps,".pdf",sep="_"),sep='/'),p,height = 10,width=10)
...
if(length(ls_list)>1){
  ...
  ggsave(paste(GP_folder,"lengthscales.pdf",sep="/"),width=12,height=8)
  ...
  ggsave(paste(GP_folder,"ranked_lengthscales.pdf",sep="/"),width=12,height=8)
  ...
  ggsave(paste(GP_folder,"average_ranked_lengthscales.pdf",sep="/"),width=10,height=6)
  ...
  sensitivities %>%
    ...
    labs(fill=NULL) +
    coord_fixed()
  ggsave(paste(GP_folder,"average_lengthscales.pdf",sep="/"),width=10,height=6)
