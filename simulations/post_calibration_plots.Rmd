...
```{r,eval=F}
threshold_score=-95000
threshold_score=quantile(total_scores$total_score,probs=seq(0,1,0.02),names = FALSE)[49]


total_scores %>% left_join(parameters) %>% select(round,param_set,total_score,parameter,unit_value) %>%
  pivot_wider(names_from=parameter,values_from = unit_value) %>%
  ungroup() %>%
  filter(total_score>=threshold_score) -> best_sets


best_sets %>% select(-total_score)%>%
  summarize(across(-c(param_set,round), mean)) -> center_best

best_sets %>% gather("parameter","unit_value",-c(round,param_set,total_score)) %>%
  left_join(center_best %>% gather("parameter","center")) %>%
  mutate(dist=abs(center-unit_value))%>%
  group_by(round,param_set)%>%
  mutate(total_dist=sum(dist)) %>%
    ungroup() %>%
  arrange(-total_dist) %>%
  mutate(keep=ifelse(param_set==best_ps & round==best_round,TRUE,FALSE))%>%
  group_by(parameter) %>% mutate(keep=dist==max(dist))%>%
  ungroup() %>%
  mutate(keep=ifelse(keep,TRUE,total_dist==max(total_dist))) %>%
  mutate(keep=ifelse(interaction(round,param_set) %in% interaction(best_round,best_ps),
                     TRUE,
                     keep))%>%
  group_by(round,param_set) %>%
  mutate(keep=sum(keep)>0)%>%
  filter(keep) -> best_sets
total_scores %>%
...
for(i in seq(1,length(myparams),1))
{
  for(j in seq(1,length(myparams),1))
  {
    if(i<j)
    {
      ...
      #              y=!!sym(myparams[j]))) +
      #   geom_point(aes(color="init"),alpha=0.5) +
      #   geom_point(data=.%>%right_join(best_sets),
      #              aes(color="selected"),alpha=0.5) +
      #   coord_fixed() +
      ...
      #ggsave(paste("~/",paste(i,"vs",j,sep="_"),".pdf",sep=""),p,height=5,width=6)
      # 
       emod_parameters %>%
        ggplot(aes(x=!!sym(myparams[i]),
                   y=!!sym(myparams[j]))) +
        ...
        geom_point(data=.%>%right_join(best_sets),
                   aes(color="selected"),alpha=0.5) +
        geom_point(data=parameter_key %>% mutate(team_default=ifelse(transformation=="log",log10(team_default),team_default))%>%
                     select(parameter,team_default)%>%pivot_wider(names_from=parameter,values_from=team_default),
                   aes(color="default"))+
        #coord_fixed() +
        labs(color="EMOD Values")+
        scale_color_manual(values=c("black","gray","red")) ->p
      print(p)
    }
  }
}
...
total_scores %>%
  ...
  geom_point(data=.%>%filter(round==0&param_set==1),
             aes(color="default"))+
  geom_point(data=.%>%filter(interaction(round,param_set)%in%interaction(best_sets$round,best_sets$param_set)),
             aes(color="top"))+
...
scores %>%
  ...
  print(bp)
  ggsave(paste(score_folder,paste("scores_total.pdf",sep="_"),sep='/'),bp,height = 5,width=8)
...
scores %>%
  ...
  print(bp)
  ggsave(paste(score_folder,paste("scores_total_by_round.pdf",sep="_"),sep='/'),bp,height = 8,width=8)
...
scores %>%
  ...
  print(bp)
  ggsave(paste(score_folder,paste("scores_objective.pdf",sep="_"),sep='/'),bp,height =8,width=10)
...
scores %>%
  ...
  ggsave(paste(score_folder,paste("scores_objective_by_round.pdf",sep="_"),sep='/'),bp,height =8,width=10)
...
temp %>%
  ...
  geom_text(aes(label=paste(ifelse(round<training_batches,paste("Init ",round),paste("Turbo ",round-training_batches+1))," # ",param_set,"\n",filltype,sep=""),
                group = weighted_ll,y=-10),
            vjust=1.2,check_overlap = T,size=3) +
  ...
  ylab("-Score") + xlab("") +
  # geom_text(data=. %>% filter(-weighted_ll>=0),
  #           aes(x=interaction("by_metric",param_set,round),
  #               label=ifelse(abs(weighted_ll)>1000,round(weighted_ll,2),""),group = weighted_ll),
  #                 position = position_stack(vjust = .5),size=3) +
  # geom_text(data=. %>% filter(-weighted_ll>=0),
  #           aes(x=interaction("by_site",param_set,round),
  #               label=ifelse(abs(weighted_ll)>1000,round(weighted_ll,2),""),group = weighted_ll),
  #                 position = position_stack(vjust = .5),size=3) +
  labs(fill=NULL,linetype=NULL) +
...
for(i in unique(track_sets$best)){
  ...
  print(bp)
  ggsave(paste("output",exp,"performance",paste(i,"performance.pdf",sep="_"),sep='/'),bp,height = 8,width=12)
}
...
tp %>%
  ...
  facet_wrap(~factor(parameter,unique(tp$parameter[param_order])),
             scales="free_y") +
  ...
  geom_segment(data=.%>%filter(parameter!="InnateImmuneDistributionFlag" & transformation=="log") %>%
                 group_by(round,parameter) %>% summarize(maximum=max(log10(as.numeric(emod_value))),
  ...
  geom_hline(data=.%>%filter(parameter!="InnateImmuneDistributionFlag" & transformation!="log"),
             aes(yintercept=as.numeric(team_default),
                 linetype="default"))+
  geom_segment(data=.%>%filter(parameter!="InnateImmuneDistributionFlag" & transformation!="log") %>%
                 group_by(round,parameter) %>% summarize(maximum=max((as.numeric(emod_value))),
  ...
  xlab("") + ylab("EMOD parameter value range") +
   theme(legend.position=c(1,0),
        ...
        legend.title = element_text(size=12)) -> pp
...
pp
  ...
  ggsave(paste(parameters_folder,"unit_values.pdf",sep='/'),pp,height = 12,width=10)
...
tp %>% ungroup() %>%
  ...
  scale_color_manual(values=c(c(rep("gray",training_batches),my_pal[seq(1,max(tp$round)+1)]))) -> pp
  pp
  ggsave(paste(parameters_folder,"unit_values_all.pdf",sep='/'),pp,height = 12,width=10)
...
#     ggpairs(mapping = aes(color=as.factor(round)))
  ...
  tp %>%
    ...
    ggplot(aes(x=unit_value,
               color=factor(ifelse(round<training_batches,
                                   paste("Init",round+1),
                                   paste("Turbo",round-training_batches+1))))) +
    geom_abline(slope=1,linetype=2)+
    facet_wrap(~factor(parameter,levels=unique(tp$parameter)[param_order])) +
    stat_ecdf(lwd = 1.2,aes(group=interaction(parameter,round))) +
    scale_color_manual(values=c(rep("gray",training_batches),my_pal[-1])) +
    labs(color=NULL) +
    theme_minimal(base_size=12)+
    theme(legend.position="right")
  
    ggsave(paste(parameters_folder,"tracking_unit_values.pdf",sep='/'),height = 10,width=18)

  
   tp %>%
    left_join(pk %>%rename(parameter=parameter_name))%>%
    #filter(round ==max(round))%>%
    ggplot(aes(color=factor(ifelse(round<training_batches,paste("Init",round+1),paste("Turbo",round-training_batches+1))))) +
    facet_wrap(~factor(parameter,levels=unique(tp$parameter)[param_order]),scales="free_x") +
    stat_ecdf(data=.%>%filter(parameter=="InnateImmuneDistributionFlag"),lwd = 1.2,
              aes(group=interaction(parameter,round),
                  x=as.numeric(as.factor(emod_value)))) +
    stat_ecdf(data=.%>%filter(parameter!="InnateImmuneDistributionFlag"),lwd = 1.2,
              aes(group=interaction(parameter,round),
                  x=ifelse(transformation=="log",
    ...
    ggsave(paste(parameters_folder,"tracking_emod_values.pdf",sep='/'),height = 10,width=18)
...
for(i in seq(1,length(parameters),2)){
  ...
  df_a  %>% 
    ...
    geom_point(aes(color=log10(-metric_score),
                   alpha=(param_set %in% unique(best_sets$param_set + best_sets$round*batch_size)))) +
  ...
  print(pp)
  ggsave(paste(paste(parameters_folder,"search_space",paste("separate",parameters[i],"x",parameters[j],sep="_"),sep="/"),".pdf",sep=''),pp,
         height=8,width=10)
}
...
for(i in seq(1,length(parameters),2)){
  ...
  print(pp)
  ggsave(paste(paste(parameters_folder,"search_space",paste("total",parameters[i],"x",parameters[j],sep="_"),sep="/"),".pdf",sep=''),pp,
         height=6,width=6)
}
