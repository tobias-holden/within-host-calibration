...
all_scores %>% arrange(weighted_ll)
...
for(i in seq(1,length(parameters),2)){
  ...
  df_a  %>% 
    ...
    scale_color_distiller(palette="Spectral") +
    labs(color="Log Score") -> pp
  
  ggsave(paste(paste(parameters_folder,"search_space",paste(parameters[i],"x",parameters[j],sep="_"),sep="/"),".png",sep=''),
         height=6,width=6)
}
...
for(s in unique(inc_df$site)){
  ...
  ggplot(aes(x=Age,y=Severe.Incidence)) +
  ggtitle(toupper(s))+
  theme_minimal(base_size=16) +
  ...
  geom_point(data=inc_ref%>%filter(Site==s),aes(x=Age,y=absolute_severe_incidence,
