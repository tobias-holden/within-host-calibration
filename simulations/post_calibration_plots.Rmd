...
tp %>% 
  ...
  mutate(emod_value=ifelse(parameter!="InnateImmuneDistributionFlag",as.numeric(emod_value),
                           emod_value)) %>%
...
total_scores %>% left_join(parameters) %>% select(round,param_set,total_score,parameter,unit_value) %>%
  ...
  ungroup() %>%
  filter(total_score>=-95000) -> best_sets
...
scores %>%
  ...
  group_by(round) %>% filter(score==max(score)) %>%
  #filter(metric!="no_blood") %>%
  ggplot(aes(#alpha=interaction(param_set,round)%in%interaction(improvements$param_set,improvements$round),
             x=interaction(param_set,round),y=-weighted_ll)) +
  geom_bar(stat="identity",position="stack",aes(fill=metric, group=weighted_ll,
                                                x=interaction(param_set,round,"by_metric")),color="white") +
  geom_bar(stat="identity",position="stack",aes(fill=site, group=weighted_ll,
                                                x=interaction(param_set,round,"by_site")),color="white") +
  theme_minimal(base_size=16) +
  ylab("-Score") + xlab("") +
  geom_text(data=. %>% filter(-weighted_ll>=0),
            aes(x=interaction(param_set,round,"by_metric"),
                label=ifelse(abs(weighted_ll)>1000,round(weighted_ll,2),""),group = weighted_ll),
                  position = position_stack(vjust = .5),size=3) +
  geom_text(data=. %>% filter(-weighted_ll>=0),
            aes(x=interaction(param_set,round,"by_site"),
                label=ifelse(abs(weighted_ll)>1000,round(weighted_ll,2),""),group = weighted_ll),
                  position = position_stack(vjust = .5),size=3) +
  geom_text(aes(label=paste(ifelse(round<training_batches,paste("Init ",round),paste("Turbo ",round-training_batches+1)),"# ",param_set,"\nby metric",sep=""),
                x=interaction(param_set,round,"by_metric"),group = weighted_ll,y=-10,vjust=1.2),
            check_overlap = T,size=3) +
   geom_text(aes(label=paste(ifelse(round<training_batches,paste("Init ",round),paste("Turbo ",round-training_batches+1)),"# ",param_set,"\nby site",sep=""),
                 x=interaction(param_set,round,"by_site"),group = weighted_ll,y=-10,vjust=1.2),
            check_overlap = T,size=3) +
  labs(fill=NULL,linetype=NULL) +
  guides(alpha="none")+
  theme(legend.position="top",
...
geom_segment(data=.%>%group_by(parameter,round) %>%
             ...
             aes(xend=factor(round),y=LQR,yend=UQR), linewidth=0.5)+
...
#     ggpairs(mapping = aes(color=as.factor(round)))
  ...
  tp %>%
    ...
    stat_ecdf(lwd = 1.2,aes(color=factor(parameter,levels=unique(tp$parameter)[param_order]),group=interaction(parameter,round))) +
