...
param_order <- c(21,1,5,10,7,6,11,12,
                               4,2,9,3,
                               13,8,14,
                               15,16,17,18,19,20)
...
scores %>%
  ...
  geom_point(data=.%>%filter(param_set==best_ps & round==best_round),
             aes(color=paste(round,"#",param_set,sep=' ')))+
  ...
  ylab("Total Unweighted Score") + xlab("#") +
  theme_minimal(base_size=16) +
...
scores %>%
  ...
  print(bp)
  ggsave(paste(score_folder,paste("scores_objective.png",sep="_"),sep='/'),bp,height=12,width=10)
...
scores %>%
  ...
  ggplot(aes(x=ps,y=-score)) +
  facet_wrap(~metric) +
  geom_point(alpha=0.1,aes(color=factor(round)))+
  geom_point(data=.%>%filter(param_set==best_ps & round==best_round),
             alpha=1,aes(shape="best"))+
  geom_hline(data=.%>%filter(ps!=1) %>% filter(param_set==best_ps & round==best_round),
             aes(yintercept=-score,linetype="best"))+
...
scores %>%
  ...
  geom_text(data=. %>% filter(-weighted_ll>=0),
            aes(label=ifelse(abs(weighted_ll)>5000,round(weighted_ll,2),""),group = weighted_ll),
  ...
  improvements %>% left_join(scores) %>% filter(round!=0)%>%
    filter(metric!="no_blood")%>%
  group_by(param_set,round) %>% mutate(score=sum(weighted_ll)) %>%
  group_by(round) %>% filter(score==max(score)) %>%
  arrange(param_set,round)%>%
  #filter(metric!="no_blood") %>%
  ggplot(aes(y=site,x=-weighted_ll)) +
  geom_bar(stat="identity",position="dodge",aes(fill=interaction(param_set,round), group=interaction(param_set,round)),color="white") +
  theme_minimal(base_size=16) +
    facet_wrap(~metric,scales="free_y") +
  xlab("Score") + ylab("") +
  # geom_text(aes(label=paste(ifelse(round<training_batches,paste("Init ",round),paste("Turbo ",round-training_batches+1)),"\n# ",param_set,sep=""),group = weighted_ll,y=-10,vjust=1.2),
  ...
  improvements %>% 
    ...
    arrange(param_set,round)%>%
    #filter(metric!="no_blood") %>%
    ggplot(aes(x=interaction(param_set,round),y=-weighted_ll)) +
    facet_wrap(~metric) +
...
unit_parameters %>% group_by(round) %>% count() %>% select(round) -> pp
...
tp %>%
  ...
  mutate(parameter=gsub("Falciparum","",parameter)) %>%
  mutate(parameter=gsub("Nonspecific Anti","Nonspecific Anti-\n",parameter)) %>%
  mutate(parameter=gsub("Severe","Severe\n",parameter))-> tp


param_levels = unique(tp$parameter)[param_order]
...
tp %>%
  ...
  pp
  ggsave(paste(parameters_folder,"emod_values.png",sep='/'),pp,height = 16,width=10)
  ...
  pp
  ggsave(paste(parameters_folder,"unit_values.png",sep='/'),pp,height = 16,width=10)
...
tp %>% ungroup() %>%
  ...
  pp
  ggsave(paste(parameters_folder,"unit_values_all.png",sep='/'),pp,height = 16,width=10)
...
group_by(round,parameter) %>% 
    ...
    geom_density(aes(x=unit_value,fill=factor(round),group=factor(round)),
                 alpha=0.2)+
...
for(i in seq(1,length(parameters),2)){
  ...
  df_a  %>% 
    ...
    facet_wrap(~metric) +
    theme_minimal(base_size=12)+
    geom_point(aes(color=log10(-metric_score)),alpha=0.2) +
...
for(i in seq(1,length(parameters),2)){
  ...
  df_a  %>% 
    ...
    ggplot(aes(x=!!sym(parameters[i]),
               y=!!sym(parameters[j]))) +
    theme_minimal(base_size=16)+
    geom_point(aes(color=log10(-total_score)),alpha=0.5) +
    geom_point(data=.%>%filter(param_set==best_ps+best_round*batch_size),
               aes(shape="best"),color="black") +
    scale_color_distiller(palette="Spectral") +
    labs(color="Log10(-Score)",shape=NULL,alpha=NULL) +
