...
def prepare_incidence_comparison_single_site(sim_df, site):
    ...
    combined_df['se'] = np.sqrt((1-combined_df['r']/1000)/combined_df['a'])
    ...
    #combined_df['se'] =  np.sqrt((1-combined_df['ref.Observations']/combined_df['ref.Trials'])/combined_df['ref.Observations'])
    #combined_df['se_95'] = np.exp(np.log())
...
def compute_incidence_likelihood1(combined_df):
...
def compute_incidence_likelihood3(combined_df):
...
def compute_inc_LL_by_site(site, numOf_param_sets):
    ...
    if len(missing_param_sets) > 0:
        print(f'Warning {site} is missing param_sets {missing_param_sets} for incidence')
...
def plot_incidence_comparison_single_site(site, param_sets_to_plot=None,plt_dir=os.path.join(manifest.simulation_output_filepath, "_plots")):
    ...
    print(combined_df[["mean_age","ref.Trials","ref.Observations","reference","a","n","r","se","LL","UL"]])
    
    plt.figure(f"{site}_incidence")
    ...
    for param_set, sdf in combined_df.groupby("param_set"):
        if param_set in param_sets_to_plot:
            plt.plot(sdf["mean_age"].to_numpy(), sdf["simulation"].to_numpy(), label=f"PS {param_set}", marker='s')
...
if __name__=="__main__":
    ...
    #plot_incidence_comparison_all_sites([1,25,491])
    #print(compute_inc_LL_for_all_sites(1000).sort_values(by="ll").to_string())
