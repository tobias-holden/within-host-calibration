...
scores <- read.csv(paste("simulations","output",exp,'all_LL.csv',sep='/'))
...
d %>%
  ...
  select(round,metric,round_worst,round_total) %>%
...
d %>%
  ...
  select(round,metric,std_score,std_total) %>%
  gather("score_type","score",4:5) %>%
  ggplot(aes(x=round,y=score)) +
...
d %>% 
  ...
  #coord_polar() +
  facet_grid(toupper(metric)~. scales="free_y") +
...
for(i in unique(dd$iivt)){
  ...
  geom_bar(aes(group=interaction(param_set,round),
                y=ll2,
                fill=plabel,
               alpha=ll2==worst),
  ...
  #          position="identity") +
  facet_wrap(~paste("round",round,"#",param_set),nrow=2) +
...
params %>% 
  ...
  select(param_set,round,emod_value,type,plabel,parameter) %>%
  group_by(param_set,round,type,plabel,parameter) %>%
  summarize(emod_value=mean(emod_value)) %>%
  pivot_wider(names_from=parameter,values_from = emod_value,id_cols = c(param_set,round)) %>%
  ungroup() %>%
  mutate(id=as.factor(interaction(param_set,round))) %>%
...
for(i in 1:nrow(pp))
{
  ...
  if(i==1){
    ...
    params <- read.csv(paste(paste(as.numeric(pp$iivt[i])-1,sep="-"),"simulations","output",exp,
                             paste('LF',pp$round[i],sep='_'),"translated_params.csv",sep='/')) %>%
              mutate(round=pp$round[i], iivt=pp$iivt[i])
  } else {
    rbind.data.frame(params,
                    read.csv(paste(paste(as.numeric(pp$iivt[i])-1,sep="-"),"simulations","output",exp,
                             paste('LF',pp$round[i],sep='_'),"translated_params.csv",sep='/')) %>%
...
inc_ref <-read.csv(paste("/reference_datasets/Cameron_age_incidence_prev.csv",
                         sep=''))
...
for(ss in 1:length(unique(ddmp$site)[4]))
{
  ...
  for(i in 1:nrow(sdmp))
  {
    ...
    tmp <- read.csv(paste("/simulations/output/",exp,"/LF_",sdmp$round[i],
  ...
  if(s=="dapelogo_2007"){
     ...
     ref_dens <- read.csv(paste("/reference_datasets/par_dens_extract_Dapelogo_2007.csv",
     ...
     ref_dens <- read.csv(paste("/reference_datasets/par_dens_extract_Laye_2007.csv",
                             sep='')) %>% rename(site=Site)
...
for(i in nrow(key))
{
  ...
  temp <- read.csv(paste("/simulations/output/",exp,
                        "/LF_",round,
                        "/SO/",site,
...
lay <- read.csv(file=paste("/reference_datasets/laye_infectiousness.csv",sep='')) %>%
  select(month,agebin,densitybin,fraction_infected_bin,freq_frac_infect,site)
# Load dapelogo reference data
dap <- read.csv(paste("reference_datasets/dapelogo_infectiousness.csv",
...
#exp <- "hyperparam_240804"
  ...
  ls_list <- list.files(glue("/simulations/output/",exp))
  ls_list <- ls_list[grepl("LS",ls_list)]

  for(ls in ls_list){
    if(!(exists("LS"))){
      LS <- read.csv(glue("/simulations/output/",exp,"/",ls))
    } else {
      tmp <- read.csv(glue("/simulations/output/",exp,"/",ls))
  ...
  ggsave(paste("lengthscales.png",sep="_"),width=12,height=8)
  ...
  ggsave(paste("ranked_lengthscales.png",sep="_"),width=12,height=8)
  ...
  ggsave(paste("average_ranked_lengthscales.png",sep="_"),width=10,height=6)
